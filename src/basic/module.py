...
def prepare(Q30,
    ...
    info_annotation = """##Infos\n##Q30 threshold:%s\n##DUPLICATE threshold:%s\n##INSERT Size threshold:%s-%s\
            ...
            \n""" % (Q30, DUPLICATE, INSERTLOW, INSERTHIGH, MAPPED, COVERAGE,
                     CONTAMINATION)

    infofile = "%s/Infos.csv" % outdir
    title = [
        "#SAMPLE", "QCFlag", "LENGTH", "GC(%)", "N(ppm)", "Q20(%)", "Q30(%)",
        "PF_READS", "CLEAN_READS", "RATIO_OF_READS(%)", "PF_BASES",
        "CLEAN_BASES", "RATIO_OF_BASES(%)", "INSERT", "DUPLICATE(%)",
        "ON_TARGET_CORE(%)", "ON_TARGET_EXT(%)", "RATIO_OF_MAPPED(%)",
        "MEAN_DEPTH", "1X_COVERAGE(%)", "10X_COVERAGE(%)", "20X_COVERAGE(%)",
        "50X_COVERAGE(%)", "20%MEAN_COVERAGE(%)", "MEAN_DEPTH_DEDUP",
        "1X_COVERAGE_DEDUP(%)", "10X_COVERAGE_DEDUP(%)",
        "20X_COVERAGE_DEDUP(%)", "50X_COVERAGE_DEDUP(%)",
        "20%MEAN_COVERAGE_DEDUP(%)", "Contamination"
    ]
    if os.path.exists(infofile):
        return title
    else:
        os.system(
            "mkdir -p TMP raw RawData %s/CleanData %s/BAM %s/Infos/InsertSize"
...
def combinedata(inid, rawdir="raw", enddir="RawData"):
    ...
    if not lis:
        ...
        cmd1 = "cat %s > %s/%s_R1.fastq.gz" % (" ".join(lis), enddir, inid)
...
def qc(inid,
    ...
    if UMIbarcode == False:
        qc_cmd = "%s -d64 -Djava.io.tmpdir=%s -Xmx30g -jar %s PE -phred33 -threads  8 %s %s %s \
                ...
                LEADING:15 TRAILING:15 SLIDINGWINDOW:5:20 AVGQUAL:20 MINLEN:36" % (
            java, javatmp_dir, trimmomatic, r1, r2, h1, inid, h2, inid,
            adapters4qc)
    else:
        qc_cmd = "%s -d64 -Djava.io.tmpdir=%s -Xmx30g -jar %s PE -phred33 -threads  8 %s %s %s \
                TMP/%s_R1.unpaired.fastq.gz %s TMP/%s_R2.unpaired.fastq.gz ILLUMINACLIP:%s:2:20:10:1:true \
                TRAILING:15 SLIDINGWINDOW:5:20 AVGQUAL:20 MINLEN:36 && \
                %s/ctDNAv3_adapter_cutter_cln2.py %s %s %s %s" % (
            java, javatmp_dir, trimmomatic, r1, r2, tmp_r1, inid, tmp_r2, inid,
            adapters4qc, scriptspath, tmp_r1, tmp_r2, h1, h2)
...
def bwa_mem(inid, bwa, ref, r1, r2, outdir="Results"):
    ...
    cmd = '%s mem -M -t 8 -R "@RG\\tID:%s\\tSM:%s\\tLB:%s\\tPL:illumina" \
            %s  %s %s | gzip > %s' % (bwa, inid, inid, inid, ref, r1, r2,
                                      samfile)
    return cmd, samfile
...
def rmdup(inid, inbam, java, javatmp_dir, picard, ref):
    ...
    cmd = '%s -Djava.io.tmpdir=%s \
            ...
            ASSUME_SORTED=true \
            CREATE_INDEX=true' % (java, javatmp_dir, picard, inbam, rmdupbam,
                                  metrics)
    return cmd, rmdupbam, metrics


def realign(inid, inbam, java, javatmp_dir, gatk, ref, indel_gsdb, indel_1000g,
            indel_mills, target):
    ...
    cmd = '%s -Djava.io.tmpdir=%s \
            ...
            ' % (java, javatmp_dir, gatk, ref, inbam, indel_gsdb, indel_1000g,
                 indel_mills, target, intervals)
    return cmd, intervals


def indelrealigner(inid, inbam, intervals, java, javatmp_dir, gatk, ref,
                   indel_gsdb, indel_1000g, indel_mills):
...
def baserecalibrator(inid, inbam, java, javatmp_dir, gatk, ref, indel_gsdb,
                     indel_1000g, indel_mills, snp_gsdb, snp_dbsnp, target):
...
def infos(inid,
    ...
    inf12, ctout = run_contamination(inid, rmdupbam, gatk4, exacVCF)
...
def combineinfo(inid,
    ...
    cmd = "%s/CombineInfos.py %s \
            %s  %s %s %s %s %s %s %s \
            >> %s/Infos.csv" % (scriptspath, inid, Q30, DUPLICATE, INSERTLOW,
                                INSERTHIGH, MAPPED, COVERAGE, CONTAMINATION,
                                DEPTH, outdir)
    return cmd
