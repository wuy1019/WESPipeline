...
def mutectv1(tumorbam, normalbam, java, javatmp, mutect, ref, fraction_contamination, intervals,
             min_qscore, cosmic, dbsnp, tmpdir, outdir="Results/mutectv1"):
...
def scalpel(tumorbam, normalbam, scalpeldir, intervals, ref, numprocs,
        max_ins_size, max_del_size, min_alt_count_tumor, min_coverage_tumor,
        max_coverage_tumor, min_coverage_normal, max_coverage_normal,
        min_phred_fisher, min_vaf_tumor, tmpdir, outdir="Results"):
...
def vardict(tumorbam, normalbam, vardictjava, vardictperlpath, ref, af,
            intervals,th, outdir="Results/mutectv1"):
...
def vcfanno(invcf, tumorid, normalid, vcf2maf, veppath, vepdata, ref, exacvcf, custom_enst, outmaf):
...
def covertMAF(inmaf, outmaf):
    header =["Hugo_Symbol", "Gene.ID", "HGVS", "Chr.start", "Chr.end", "Reference_Allele",
             "Tumor_Seq_Allele1", "Tumor_Seq_Allele2", "Variant_Classification", "dbSNP_RS",
             "t-AF", "t_DP(ref:alt)", "n-AF", "n_DP(ref:alt)", "Entrez_Gene_Id", "NCBI_Build",
             "Strand", "Tumor_Sample_Barcode", "Matched_Norm_Sample_Barcode", "Match_Norm_Seq_Allele1",
             ...
             "Matched_Norm_Sample_UUID", "HGVSp", "Transcript_ID", "all_effects", "Allele",
             ...
             "DISTANCE", "STRAND_VEP", "SYMBOL", "SYMBOL_SOURCE", "HGNC_ID", "BIOTYPE", "CANONICAL",
             "CCDS", "ENSP", "SWISSPROT", "TREMBL", "UNIPARC", "SIFT", "PolyPhen", "INTRON", "DOMAINS",
             "AF", "AFR_AF", "AMR_AF", "ASN_AF", "EAS_AF", "EUR_AF", "SAS_AF", "AA_AF", "EA_AF",
             ...
             "variant_id", "variant_qual", "ExAC_AF_Adj", "ExAC_AC_AN_Adj", "ExAC_AC_AN",
             "ExAC_AC_AN_AFR", "ExAC_AC_AN_AMR", "ExAC_AC_AN_EAS", "ExAC_AC_AN_FIN", "ExAC_AC_AN_NFE",
             "ExAC_AC_AN_OTH", "ExAC_AC_AN_SAS", "ExAC_FILTER", "gnomAD_AF", "gnomAD_AFR_AF",
    ...
    for key, val in mafdic.items():
        ...
        if val["HGVSp_Short"]:
            ...
            val["HGVS"] = val["HGVSc"]
        val["Chr.start"] = "{}:{}".format(val["Chromosome"], val["Start_Position"])
        val["Chr.end"] = "{}:{}".format(val["Chromosome"], val["End_Position"])
        val["t-AF"] = round(float(val["t_alt_count"])/float(val["t_depth"]), 2)
        val["n-AF"] = round(float(val["n_alt_count"])/float(val["n_depth"]), 2)
        val["t_DP(ref:alt)"] = "{}({}:{})".format(val["t_depth"], val["t_ref_count"], val["t_alt_count"])
        val["n_DP(ref:alt)"] = "{}({}:{})".format(val["n_depth"], val["n_ref_count"], val["n_alt_count"])
